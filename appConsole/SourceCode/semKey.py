    ...
    while True:
        ...
        match answer:
            case 1:
                ...
                subprocess.run('takeown /f "%temp%" /r /d y', shell=True)
                subprocess.run('RD /S /Q %temp%', shell=True)
                subprocess.run('MKDIR %temp%', shell=True)
                subprocess.run('takeown /f "%temp%" /r /d y', shell=True, )
                subprocess.run('takeown /f "C:\Windows\Temp" /r /d y')
                subprocess.run('RD /S /Q C:\Windows\Temp', shell=True)
                subprocess.run('MKDIR C:\Windows\Temp', shell=True)
                subprocess.run('takeown /f "C:\Windows\Temp" /r /d y', shell=True)
                
                subprocess.run('cd C:/ & del *.log /a /s /q /f', shell=True)
                
                subprocess.run('net stop wuauserv', shell=True)
                subprocess.run('net stop UsoSvc', shell=True)
                subprocess.run('rd /s /q C:\Windows\SoftwareDistribution', shell=True)
                subprocess.run('md C:\Windows\SoftwareDistribution', shell=True)
                
                subprocess.run('FOR %F IN ("%SystemRoot%\servicing\Packages\Microsoft-Windows-GroupPolicy-ClientTools-Package~*.mum") DO (DISM /Online /NoRestart /Add-Package:"%F")', shell=True)
                subprocess.run('FOR %F IN ("%SystemRoot%\servicing\Packages\Microsoft-Windows-GroupPolicy-ClientExtensions-Package~*.mum") DO (DISM /Online /NoRestart /Add-Package:"%F")', shell=True)
                ...
                subprocess.run('rd /s /q "C:\Program Files\*.msi"', shell=True)
